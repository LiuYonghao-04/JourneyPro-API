...
const POST_TARGET = Math.max(120, Number(process.env.SEED_POSTS_TARGET || 1500));
const USER_TARGET = Math.max(8, Number(process.env.SEED_USERS_TARGET || 28));
const POI_CANDIDATES = Math.max(POST_TARGET, Number(process.env.SEED_POI_CANDIDATES || 5200));
const COMMENTS_MIN = Math.max(1, Number(process.env.SEED_COMMENTS_MIN || 1));
const COMMENTS_MAX = Math.max(COMMENTS_MIN, Number(process.env.SEED_COMMENTS_MAX || 3));
const NOW = Date.now();
...
const fetchCandidatePois = async (conn) => {
  const [rows] = await conn.query(
    `
      ...
      WHERE p.source='OSM'
        AND p.image_url REGEXP '^(http|https)://'
...
const main = async () => {
  ...
  try {
    ...
    for (let i = 0; i < POST_TARGET; i += 1) {
      ...
      const poiImages = unique([poi.image_url, ...(poiPhotoMap.get(Number(poi.id)) || [])]).slice(0, 6);
      const postImages = poiImages.length ? poiImages : [poi.image_url];
      const title = buildPostTitle(poi).slice(0, 100);
      const content = buildPostText(poi);
      const tags = buildTags(poi);
      const likeCount = randInt(5, 240);
      const favoriteCount = randInt(2, 120);
      const viewCount = randInt(80, 3200);
      ...
      for (const parentId of rootCommentIds) {
        const shouldReply = Math.random() < 0.62;
        if (!shouldReply) continue;
        const replyCount = Math.random() < 0.22 ? 2 : 1;
      ...
      JSON.stringify(
        {
          ...
          pois_used: Math.min(pois.length, POST_TARGET),
